...
export default function BreastfeedingPainScreen({ navigation }: Props) {
  const { t } = useTranslation();

  const setField = useOnboardingStore((s) => s.setField);
  const stored = useOnboardingStore((s) => s.data.breastfeedingPain) as
    | RatingScale
    | undefined;
  const babyName = useOnboardingStore((s) => s.data.babyName);

  const title = t('onboarding.breastfeedingPain', {
    name: babyName || t('onboarding.defaultName'),
  });

  // visible slider value (number), default neutral = 3
  ...
  const handleSlidingComplete = (val: number) => {
    ...
    setValue(rounded);
    // persist immediately
    setField('breastfeedingPain', rounded);
  ...
  const handleNext = () => {
    ...
    setField('breastfeedingPain', final);
    navigation.navigate('MoodIssues');
  };
  ...
  return (
    <OnboardingScreenLayout
      ...
      onPressSkip={() => navigation.navigate('MoodIssues')}
    >
