...
type Props = NativeStackScreenProps<OnboardingStackParamList, 'MilkSupply'>;
type RatingScale = 1 | 2 | 3 | 4 | 5;

export default function MilkSupplyScreen({ navigation }: Props) {
  const { t } = useTranslation();

  const setField = useOnboardingStore((s) => s.setField);
  const stored = useOnboardingStore((s) => s.data.milkSupplyConcern) as
    | RatingScale
    | undefined;
  const babyName = useOnboardingStore((s) => s.data.babyName);

  const title = t('onboarding.milkSupply', {
    name: babyName || t('onboarding.defaultName'),
  });

  // Slider numeric state (always has a visible value)
  const [sliderVal, setSliderVal] = useState<number>(stored ?? 3);
  // Persisted rating (undefined until user slides or stored exists)
  ...
  const handleSlidingComplete = (val: number) => {
    ...
    setField('milkSupplyConcern', rounded);
  ...
  const handleNext = () => {
    ...
    setField('milkSupplyConcern', final);
    navigation.navigate('BreastfeedingPain');
  };
  ...
  return (
    <OnboardingScreenLayout
      ...
      onPressSkip={() => navigation.navigate('BreastfeedingPain')}
    >
