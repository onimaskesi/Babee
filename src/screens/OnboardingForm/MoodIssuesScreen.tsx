...
type Props = NativeStackScreenProps<OnboardingStackParamList, 'MoodIssues'>;
type RatingScale = 1 | 2 | 3 | 4 | 5;

export default function MoodIssuesScreen({ navigation }: Props) {
  const { t } = useTranslation();

  // store accessor and existing (possibly saved) value
  const setField = useOnboardingStore((s) => s.setField);
  const stored = useOnboardingStore((s) => s.data.moodIssues) as
    | RatingScale
    | undefined;
  const babyName = useOnboardingStore((s) => s.data.babyName);

  // Local slider numeric value (visible) and persisted value
  const [sliderVal, setSliderVal] = useState<number>(stored ?? 3);
  const [value, setValue] = useState<RatingScale | undefined>(stored);

  // Title uses baby name fallback
  const title = t('onboarding.moodIssues', {
    name: babyName || t('onboarding.defaultName'),
  });

  useEffect(() => {
    // keep local state in sync with any stored value
    if (stored !== undefined) {
      setSliderVal(stored);
      setValue(stored);
    }
  ...
  // Continue to next screen (HealthNotes)
  const handleNext = () => {
    const final = value ?? (Math.round(sliderVal) as RatingScale);
    setField('moodIssues', final);
    navigation.navigate('HealthNotes');
  };
  ...
  return (
    <OnboardingScreenLayout
      ...
      buttonDisabled={false} // slider always has a value (defaults to 3)
      onPressSkip={() => navigation.navigate('HealthNotes')}
    >
      <View style={styles.wrapper}>
        {/* Big value circle + descriptor */}
